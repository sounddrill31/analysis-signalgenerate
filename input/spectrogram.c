                 ...
                 double varargout_3[1845])
{
  ...
  int iCol;
  int k;
#pragma omp parallel for num_threads(omp_get_max_threads()) private(r, r1, i, k)

  for (iCol = 0; iCol < 1845; iCol++) {
    i = iCol << 9;
    for (k = 0; k <= 1022; k += 2) {
      r1 = _mm_loadu_pd(&dv[k]);
      r = _mm_loadu_pd(&x[k + i]);
      r = _mm_mul_pd(r1, r);
      _mm_storeu_pd(&c[k + (iCol << 10)], r);
    }
  }
  double f[1024];
  computeDFT(c, varargin_4, b_y1, f);
  formatSpectrogram(b_y1, varargin_4, varargout_1, varargout_2);
  r2 = _mm_set1_pd(512.0);
  for (i1 = 0; i1 <= 1842; i1 += 2) {
    ...
    _mm_storeu_pd(&varargout_3[i1],
                  _mm_div_pd(_mm_add_pd(_mm_mul_pd(r2, r3), r2),
