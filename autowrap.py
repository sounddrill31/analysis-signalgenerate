...
def parse_argument(arg_str):
    ...
    # Force MATLAB to respect our inputs
    if name == 'x':
        return {'type': 'array_in_emx', 'name': name}
    if name == 'Fs':
        return {'type': 'scalar_in', 'name': name}
    
    # Catch fixed-size outputs generated by MATLAB optimizations
    array_match = re.search(r'\[\s*(\d+)\s*\]', arg)
    if array_match:
        return {'type': 'array_out_fixed', 'name': name, 'size': int(array_match.group(1))}
...
"""
    ...
    for p in parsed_args:
        ...
        if name == 'x':
            ...
            # THE FIX: Use MATLAB's official initialization API for outputs
            cpp += f"    emxArray_real_T *{name};\n"
            cpp += f"    emxInitArray_real_T(&{name}, 2);\n"
    ...
    for p in outputs:
        ...
        if p_type == 'array_out_fixed':
            ...
            cpp += f"        result.set(\"{name}\", {name}_js);\n"
            cpp += f"    }} else {{\n"
            cpp += f"        result.set(\"{name}\", val::global(\"Float64Array\").new_(0));\n"
            cpp += f"    }}\n"
